...
impl MonoAI {
  ...
  /// Create OpenAI client with API key and model name
    pub fn openai_compatible(base_url:String, api_key: String, model: String) -> Self {
        Self {
            provider: Provider::OpenAICompatible(OpenAICompatibleClient::new(base_url,api_key, model)),
        }
    }
    ...
    pub async fn add_tool(&mut self, tool: Tool) -> Result<(), Box<dyn Error>> {
        match &mut self.provider {
            ...
            Provider::OpenAICompatible(client)=>client.add_tool(tool).await
        }
    }
    ...
    pub async fn is_fallback_mode(&self) -> bool {
        match &self.provider {
            ...
            Provider::OpenAI(client) => client.is_fallback_mode().await,
             Provider::OpenAICompatible(client) => client.is_fallback_mode().await,
    ...
    pub fn set_debug_mode(&mut self, debug: bool) {
        match &mut self.provider {
            ...
            Provider::OpenAI(client) => client.set_debug_mode(debug),
                        Provider::OpenAICompatible(client) => client.set_debug_mode(debug),
    ...
    pub fn debug_mode(&self) -> bool {
        match &self.provider {
            ...
            Provider::OpenAI(client) => client.debug_mode(),
                        Provider::OpenAICompatible(client) => client.debug_mode(),
    ...
    pub async fn supports_tool_calls(&self) -> Result<bool, Box<dyn Error>> {
        match &self.provider {
            ...
            Provider::OpenAI(client) => client.supports_tool_calls().await,
                        Provider::OpenAICompatible(client) => client.supports_tool_calls().await,
    ...
    pub async fn send_chat_request(
        ...
        match &self.provider {
            ...
            Provider::OpenAI(client) => client.send_chat_request(messages).await,
                        Provider::OpenAICompatible(client) => client.send_chat_request(messages).await,
    ...
    pub async fn send_chat_request_no_stream(
        ...
        match &self.provider {
            ...
            Provider::OpenAI(client) => client.send_chat_request_no_stream(messages).await,
                        Provider::OpenAICompatible(client) => client.send_chat_request_no_stream(messages).await,
    ...
    pub async fn show_model_info(&self, model_name: &str) -> Result<ModelInfo, Box<dyn Error>> {
        match &self.provider {
            ...
            Provider::OpenAI(_) => Err("show_model_info is not supported for OpenAI provider".into()),
                        Provider::OpenAICompatible(_) => Err("show_model_info is not supported for OpenAI provider".into()),
    ...
    pub async fn pull_model(&self, model_name: &str) -> Result<(), Box<dyn Error>> {
        match &self.provider {
            ...
            Provider::OpenAI(_) => Err("pull_model is not supported for OpenAI provider".into()),
                        Provider::OpenAICompatible(_) => Err("pull_model is not supported for OpenAI provider".into()),
    ...
    pub async fn pull_model_stream(
        ...
        match &self.provider {
            ...
            Provider::OpenAI(_) => Err("pull_model_stream is not supported for OpenAI provider".into()),
                        Provider::OpenAICompatible(_) => Err("pull_model_stream is not supported for OpenAI provider".into()),
    ...
    pub async fn handle_tool_calls(&self, tool_calls: Vec<ToolCall>) -> Vec<Message> {
        match &self.provider {
            ...
            Provider::OpenAI(client) => client.handle_tool_calls(tool_calls).await,
                        Provider::OpenAICompatible(client) => client.handle_tool_calls(tool_calls).await,
    ...
    pub async fn process_fallback_response(&self, content: &str) -> (String, Option<Vec<ToolCall>>) {
        match &self.provider {
            ...
            Provider::OpenAI(client) => client.process_fallback_response(content).await,
                        Provider::OpenAICompatible(client) => client.process_fallback_response(content).await,
    ...
    pub fn model(&self) -> &str {
        match &self.provider {
            ...
            Provider::OpenAI(client) => &client.model,
                        Provider::OpenAICompatible(client) => &client.model,
